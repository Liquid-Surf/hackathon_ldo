...
export abstract class Theme {
    ...
    abstract createDefaultTemplate(label: string, value: Term | null, required: boolean, editor: Editor, template?: ShaclPropertyTemplate): HTMLElement;
    ...
    abstract createImageEditor(label: string, value: Term | null, required: boolean, template: ShaclPropertyTemplate): HTMLElement
...
export function fieldFactory(template: ShaclPropertyTemplate, value: Term | null): HTMLElement {
    if (template.config.editMode) {
        ...
        switch (template.datatype?.value.replace(PREFIX_XSD, '')) {
            ...
            case 'image':
                return template.config.theme.createImageEditor(template.label, value, required, template)
            case 'boolean':
